<template>
    <div class="flex flex-row-reverse md:flex-row items-center justify-end py-4 gap-5">
        <div class="flex items-center gap-2">
            <Button class="gap-2" @click="emits('create')">
                <CirclePlus />
            </Button>
            ...
            <TableBody>
                <template v-if="table.getRowModel().rows?.length">
                    <TableRow v-for="row in table.getRowModel().rows" :key="row.id"
                        ...
                        <TableCell class="p-3" v-for="cell in row.getVisibleCells()" :key="cell.id">
