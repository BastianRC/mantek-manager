...
<!-- <template>
    ...
    <BackgroundCard v-if="role" id="info" title="Informacion" :icon="Info">
        <UpdateRoleNameForm @submit="onUpdateRoleName" :loading="isPendingRoleName" :role="role" />
    </BackgroundCard>

    <BackgroundCard v-if="role && permissions" id="permissions" title="Permisos" :icon="ShieldAlert">
        <UpdateRolePermissionsForm @submit="onUpdateRolePermissions" :loading="isPendingRolePermissions" :role="role" :permissions="permissions" />
    </BackgroundCard>

</template>

<script setup lang="ts">
import { Info, ShieldAlert, UserRoundPen } from 'lucide-vue-next';
import BackgroundCard from '~/components/custom/Card/BackgroundCard.vue';
import Progress from '~/components/ui/progress/Progress.vue';
import { useGetRoleById } from '../composables/useGetRoleById';
import { useUpdateRole } from '../composables/useUpdateRole';
import UpdateRoleNameForm from '../components/UpdateRoleNameForm.vue';
import UpdateRolePermissionsForm from '../components/UpdateRolePermissionsForm.vue';
import { useGetPermissionsList } from '../composables/useGetPermissionsList';
...
const { data: role, isFetching, suspense: suspense } = useGetRoleById(selectedId)
const { data: permissions } = useGetPermissionsList()

const { mutate: updateRoleName, isPending: isPendingRoleName } = useUpdateRole()
const { mutate: updateRolePermissions, isPending: isPendingRolePermissions } = useUpdateRole()

const onUpdateRoleName = (event: any) => {
    updateRoleName({ id: selectedId.value!, payload: { name: event.name } })
}

const onUpdateRolePermissions = (event: any) => {
    updateRolePermissions({ id: selectedId.value!, payload: { permissions: event.permissions } })
}
